...
import com.guilin.elasticsearch.demo.Base;
...
public class GroovyScriptTests  extends Base{
    ...
    public void testGroovyBigDecimalTransformation() {
        esClient.prepareIndex("test", "doc", "1").setSource("foo", 5).setRefresh(true).get();
    ...
    public void assertScript(String script) {
        String source = "{\"query\": {\"match_all\": {}}," +
                ...
                "; 1\", \"type\": \"number\", \"lang\": \"groovy\"}}}";
        System.out.println(source);
        SearchResponse resp = esClient.prepareSearch("test")
...
//        esClient.prepareIndex("test", "doc", "1").setSource("foo", "quick brow fox jumped over the lazy dog", "bar", 1).get();
//        esClient.prepareIndex("test", "doc", "2").setSource("foo", "fast jumping spiders", "bar", 2).get();
//        esClient.prepareIndex("test", "doc", "3").setSource("foo", "dog spiders that can eat a dog", "bar", 3).get();
//        refresh();
//
//        // doc[] access
//        SearchResponse resp = esClient.prepareSearch("test").setQuery(functionScoreQuery(matchAllQuery())
//.add(
